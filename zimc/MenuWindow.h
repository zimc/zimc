...
public:
	...
	int   EventClickMenu(LPCTSTR tszMenuName)
	{
		static LPCTSTR s_tszMenuNameTable[] = 
		{
			_T("OpenMenu"), 
			_T("ModifyMenu"), 
			_T("DeleteMenu"), 
		...
		{
			if(_tcscmp(tszMenuName, s_tszMenuNameTable[i]) == 0)
			{
				return i;
			}
		}
    ...
    void Notify(TNotifyUI& msg)
    {
        if( msg.sType == _T("itemselect") ) {
            ...
            //if( msg.pSender->GetName() == _T("DeleteMenu") ) {
            //    if( m_pOwner ) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_Delete"), 0, 0, true);
            //}
			if(m_pOwner && msg.pSender->GetName()) m_pOwner->GetManager()->SendNotify(
				m_pOwner, _T("menuclick"), EventClickMenu(msg.pSender->GetName()), 0, true);
        }
    }
    ...
    LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        CControlUI* pRoot = builder.Create(_T("ziuser_menu.xml"), (UINT)0, NULL, &m_pm);
